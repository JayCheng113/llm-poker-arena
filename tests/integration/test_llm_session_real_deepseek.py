...
pytestmark = pytest.mark.skipif(
    os.getenv("DEEPSEEK_INTEGRATION_TEST") != "1" or not os.getenv("DEEPSEEK_API_KEY"),
...
def test_real_deepseek_chat_plays_six_hands(tmp_path: Path) -> None:
    ...
    sess = Session(config=cfg, agents=agents, output_dir=tmp_path, session_id="real_deepseek_smoke")
    ...
    assert all(it.get("reasoning_artifacts", []) == [] for it in rec["iterations"])
