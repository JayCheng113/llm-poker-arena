...
def _run_small_session(tmp_path: Path) -> Path:
    ...
    sess = Session(config=cfg, agents=agents, output_dir=sess_dir, session_id="smoke")
...
def test_duckdb_can_read_actions_view(
    ...
    with open_session(sess_dir, access_token=PRIVATE_ACCESS_TOKEN) as con:
        ...
        cols = {row[0] for row in con.sql("DESCRIBE actions").fetchall()}
        # Required fields for VPIP/PFR/action_distribution SQL.
        for required in ("seat", "hand_id", "street", "is_forced_blind", "final_action"):
...
def test_duckdb_can_access_final_action_type_via_dot_notation(
    ...
    with open_session(sess_dir, access_token=PRIVATE_ACCESS_TOKEN) as con:
        ...
        rows = con.sql("SELECT DISTINCT final_action.type FROM actions").fetchall()
