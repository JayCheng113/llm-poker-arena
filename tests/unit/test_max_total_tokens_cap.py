...
def test_cap_none_runs_full_session(tmp_path: Path) -> None:
    ...
    sess = Session(config=cfg, agents=agents, output_dir=tmp_path, session_id="no_cap")
...
def test_cap_aborts_after_hand_when_exceeded(tmp_path: Path) -> None:
    ...
    cfg = SessionConfig(
        ...
        bb=100,
        num_hands=12,
        max_utility_calls=5,  # 12 hands; cap will trip earlier
        enable_math_tools=False,
        enable_hud_tool=False,
        rationale_required=False,
        opponent_stats_min_samples=30,
    ...
    script = MockResponseScript(responses=tuple(_bigfold(f"t{i}") for i in range(200)))
    ...
    sess = Session(config=cfg, agents=agents, output_dir=tmp_path, session_id="cap_trip")
    asyncio.run(sess.run())
    meta = json.loads((tmp_path / "meta.json").read_text())
...
def test_cap_above_session_total_does_not_abort(tmp_path: Path) -> None:
    ...
    agents = [RandomAgent() for _ in range(6)]
    sess = Session(config=cfg, agents=agents, output_dir=tmp_path, session_id="cap_unreached")
    asyncio.run(sess.run())
    meta = json.loads((tmp_path / "meta.json").read_text())
    assert meta["total_hands_played"] == 6
    assert meta.get("stop_reason") in (None, "completed")
