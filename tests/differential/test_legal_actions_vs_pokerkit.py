...
def test_our_legal_names_match_pokerkit_predicates(rng_seed: int, hand_id: int) -> None:
    ...
    pk_can_check_or_call = (
        bool(raw.can_check_or_call()) if hasattr(raw, "can_check_or_call") else False
    )
    pk_can_complete = (
        bool(raw.can_complete_bet_or_raise_to())
        if hasattr(raw, "can_complete_bet_or_raise_to")
        else False
    )
